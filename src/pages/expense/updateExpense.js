...
import { Btn, CInput, Txt,HAND_ACCOUNT, get_error_string, CheckBox, get_date, RadioBtnCollection, Check_Positive_Number_Error, Check_OBJ_Required_Error, GetItemID, Tost_Massages, setSearchPickerDefaulValue, getDefaultLocationParams, setSearchPickerDefaulValueForAccount  } from '../../components/general';
...
class Page extends Component {
    ...
    componentDidMount(){
      if(this.props.location && this.props.location.state){
        if(this.props.location.state.data && this.props.location.state.data.id){
            ...
            this.setState({
                from:setSearchPickerDefaulValueForAccount({name:data.user_from_name,id:data.from_id}),
  ...
  clear_state = ()=>{
    ...
    onSubmit = ()=>{
        ...
        this.setState({e_amount,e_remarks,e_from});
        if(!e_amount && !e_remarks && !e_from ){
            this.setState({loading:true});
            const post = {
                id:s.id,
                account_id:GetItemID(s.from),
                amount:Number(s.amount),
                remarks:s.remarks,
                date:this.state.date?new Date(this.state.date):new Date(),
                project_id:GetItemID(s.project),
            }
            Post("/expense/editExpense",post,(st,res)=>{
              if(st){
                this.clear_state();
         ...
         <div style={{}}>
          ...
          <div className='card' style={{}}>
              <div className='fl' style={{marginBottom:0,overflow:"hidden",alignItems:"center",borderRadius:"6px 6px 0px 0px"}} >
                <h4  class="card-title mg-b-2 mt-3 ml-3 mb-3" style={{flex:1,color:"#000"}}>UPDATE EXPENSE</h4>
              </div>
